...
const viewers = new Set(); // WebSockets sin JOIN (solo escuchan)
...
function broadcast(data) {
    ...
    for (const vws of viewers) {
        if (vws.readyState === WebSocket.OPEN) vws.send(json);
    }
}
...
async function iniciarServidor() {
    ...
    try {
        ...
        wss.on('connection', (ws) => {
            ...
            viewers.add(ws); // toda conexión empieza como viewer
            ...
            ws.on('message', (data) => {
                ...
                switch (message.type) {
                    case 'JOIN': {
                        ...
                        viewers.delete(ws); // ya no es viewer, es jugador
